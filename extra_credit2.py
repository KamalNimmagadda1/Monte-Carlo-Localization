...
class Particle(object):
	...
	for i in range(len(scan.ranges)):
		...
		o = np.exp(-k/denom)/t1
		if(scan.ranges[i]<=sim_ranges[i]):
			ne = np.power((1-np.exp(-lmd*sim_ranges[i])),-1)
			e = ne*lmd
			l = e*(np.exp(-lmd*scan.ranges[i]))
			n = np.power(sim_ranges[i],-1)
		else:
			l = 0
			n = 0
		if(scan.ranges[i]==sim_ranges[i]):
			...
			m = 0
		r = (np.power(l,2)+np.power(m,2)+np.power(n,2)+np.power(o,2))/(l+m+n+o)
		p.append(r)
