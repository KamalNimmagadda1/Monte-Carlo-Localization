...
class Particle(object):
	...
	xvel += np.random.normal(0, (a3*LINEAR_MODEL_VAR_X + a4*ANGULAR_MODEL_VAR))
	yvel += np.random.normal(0, (a3*LINEAR_MODEL_VAR_Y + a4*ANGULAR_MODEL_VAR))

	X = np.array([dt*xvel, dt*yvel])
	self.map_T_particle.translation += RotateBy(X, self.map_T_particle.rotation)

	w = (odom_msg.twist.twist.angular.z + np.random.normal(0, (a1*ANGULAR_MODEL_VAR + a2*((LINEAR_MODEL_VAR_Y + LINEAR_MODEL_VAR_X)/4))))*dt
